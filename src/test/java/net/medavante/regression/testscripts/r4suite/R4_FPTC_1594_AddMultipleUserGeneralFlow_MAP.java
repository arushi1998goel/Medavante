...
*  Test Case Id: FP-TC-1594 || Test Case Name: Add Multiple Site Raters - General Flow 
   * Pre-Requisite:1. At least one Study and Site exists for the test. At least one Role Group is created for Site Raters and Other with site System Roles in the Study
   * MA Admin User without claim to view and manage Site Raters and Other exists.
   * MA Admin User with the claim to view and manage Site Raters and Other exists.
   * At least one .xlsx spreadsheet with 2 site users data without validation errors exists. Make sure the site users have unique data First/Last Name and mail.
   * First user has System Role form Site Raters role group.
   * Second user has System Role form Other role group.	
* ========================================================================= 
*/
...
public class R4_FPTC_1594_AddMultipleUserGeneralFlow_MAP extends BaseTest
{

	private String study;
	
	@Factory(dataProvider= "Browsers",dataProviderClass= DataProviders.class)
	public R4_FPTC_1594_AddMultipleUserGeneralFlow_MAP(String browser) {
		super(browser);
	}
	...
	@Test(description="FP-TC-1594 ---Add Multiple User General Flow ")
	public void R3_FPTC_1594_AddMultipleUserGeneralFlow() throws InterruptedException, Exception
	{
        ...
        reportLog("3.3: On-boarding grid is displayed");
        ratersDetailsPage.verifyOnBoardingTabDisplayed();
        
         reportLog("3.4: Bulk Load option is displayed" );
        ratersDetailsPage.bulkUploadOptionDisplayed();
        
        reportLog("4.0: Select Bulk Load -> Upload File option");
        ...
        reportLog("9.0: Select Bulk Load -> Upload File option");
        ratersDetailsPage.selectActionToAddMultipleSiteRaters();
        
        reportLog("9.1: A modal window for adding Multiple user is displayed");
        ratersDetailsPage.verifyAddMultipleSiteRatersWindowDisplayed();
        ...
        reportLog("11.1: Modal window with the confirmation message 'these users will not be added! Are you sure?' is displayed");
        ...
        reportLog("13.2:'Review each user information one by one before adding' option is displayed" );
        ...
        reportLog("14.0: Activate 'Review each user information one by one before adding' option -> "
     ...
     // ratersDetailsPage.selectSystemRoleOnAddMultipleUserPopUp();
        ratersDetailsPage.clickOnNextButton();
     // ratersDetailsPage.selectFacility(Constants.FacilityName, Constants.Add_MultipleSiteRater_PopUp);

        reportLog("14.1: User information (without validation errors) is displayed in corresponding modal window");
        ratersDetailsPage.verifySiteRaterInformationWithoutValidationErrorsDisplayed(
        		Constants.FilledrequiredColor,Constants.FilledrequiredColor_1,Constants.FilledrequiredColor_2,
        		Constants.FilledrequiredColor_1);
        ...
        reportLog("16.1: User information is displayed in corresponding modal window");
        ratersDetailsPage.verifyAddMultipleSiteRatersWindowDisplayed();
        
        reportLog("16.2: Delete all existing Facilities for user");
        ...
        reportLog("16.5: Sites drop-down is empty");
        ratersDetailsPage.verifySiteDropDownIsEmpty();

        reportLog("16.6: System Role drop-down is displayed with selected data");
        ratersDetailsPage.verifySystemRoleDropDownIsDisplayedWithSelectedData(Constants.System_Role);
        
        reportLog("16.7: Next control is disabled");
        ratersDetailsPage.verifyNextcontrolDisabled();
        
        reportLog("16.8: Cancel control is enabled");
        ratersDetailsPage.verifyCancelbuttonEnabled();
        
        reportLog("16.9: Back control is enabled");
        ratersDetailsPage.verifyBackControlIsEnabled();
        ...
        reportLog("18.0: Select an action to proceed to the Next step for all user");
        ratersDetailsPage.clickOnNextButton();
        
        reportLog("18.1: Modal window with message  2 user are ready to be added.Please select 'Finish' to load Users to the system'is displayed (according to Pr. #4)");
