...
*  pre-qualification :1. At least 2 active Studies and Site exist for the test. Role Groups for Site Rater/Other are configured for each Study with Site System Role.
                      2. At least 2 Site Users (Site User_1,2) associated with Study_1 exist
                      3. At least 3 Site Users associated with the Study_2 PR#1 and have Global Review status:
                        - Site User_3 has the same First name + Last name as Site User_1
                        - Site User_4 has the same additional email as Site User_2
                        - Site User_5 has the primary email which match with additional email of Site User_1
                      4. User with claim to allow Review User Match exists
* ======================================================================================================================================= 
...
public class R4_FPTC_2195_ActionForPartialMatchedUsersGlobalReview_MAP extends BaseTest {

	private String study,primaryEmail1,primaryEmail2,additionalEmail1,additionalEmail3;
	
	@Factory(dataProvider = "Browsers", dataProviderClass = DataProviders.class)
	public R4_FPTC_2195_ActionForPartialMatchedUsersGlobalReview_MAP(String browser) {
		super(browser);
	}
	...
	public void getTestData() throws Exception {
		...
		/***********************************************/
		reportLog("Creating User One For Prequisite");
		...
		/***********************************************/
		reportLog("Creating User Two For Prequisite");
		...
		/***********************************************/
		reportLog("Creating User Three For Prequisite");
	...
	@Test(description="FP-TC-2195--Action for Partial Matched Users: Global Review- V24")
	public void R4_FPTC_2195_ActionForPartialMatchedUsersGlobalReview() throws Exception
	{
		...
		reportLog("3.6: Site User_3 is displayed in the User under review section with highlighted matched users details");
		...
		reportLog("3.7: Site User_1 is displayed in the Existing Users section with highlighted matched users details");
		...
		reportLog("4.2: Site User_3 is displayed in the On-boarding grid");
		ratersDetailsPage.verifyOnBoardingGridIsDisplayed();
		ratersDetailsPage.verifySiteRaterIsDisplayed(Constants.RaterForReview_3);
		
		reportLog("5.0: Select row for the Site User_3");
		ratersDetailsPage.applyNameFilterUnderTrackingTab(Constants.RaterForReview_3);
		ratersDetailsPage.applyFilterUnderStatusTab(Constants.status);
		ratersDetailsPage.selectRater(Constants.RaterForReview_3);
		...
		reportLog("5.6: Select Discard User control");
		ratersDetailsPage.selectDiscardControlButton();
		
		reportLog("6.1: Site User_3 is not displayed in the grid");
		ratersDetailsPage.verifyRaterIsDeletedOrNotDisplayed(Constants.RaterForReview_3);

		reportLog("7.0: Select row for Site User_4");
		...
		reportLog("7.1: Site User_4 is selected");
		...
		reportLog("Site User_4 is displayed in the User under review section with highlighted matched users details");
		...
		reportLog("Site User_2 is displayed in the Existing Users section with highlighted matched users details");
		...
		reportLog("8.2: User cannot select any existing Site User");
        ...
        reportLog("9.1: Status Invited is applied for Site User_4");
        ratersDetailsPage.verifyRaterStatusIsDisplayed(Constants.RaterForReview_4,Constants.Status);
        
        reportLog("10.0: Navigate to Site User_4 view history page");
        ratersDetailsPage.applyNameFilterUnderTrackingTab(Constants.RaterForReview_4);
		ratersDetailsPage.selectRater(Constants.RaterForReview_4);
		ratersDetailsPage.selectActionControl();
		ratersDetailsPage.clickOnViewHistoryLink();
		
        reportLog("10.1: Appropriate actions are displayed in the View History page for Site User_4");
        ratersDetailsPage.verifyNewEntryDisplayed();
		ratersDetailsPage.selectBackButton();
        
        reportLog("11.0: Select row for Site User_5");
        ...
        reportLog("11.1: Site User_5 is selected");
        ...
        reportLog("11.4: Site User_5 is displayed in the User under review section with highlighted matched users details");
        ratersDetailsPage.verifyFieldHighlightedForUnderReviewUser(Constants.samePrimaryEmail,Constants.HighlightedColor);
		ratersDetailsPage.verifyUserUnderReviewFieldDisplayedHighlighgtedForMatchedFields(Constants.facility,Constants.study, 
				Constants.site,Constants.personType,Constants.HighlightedColor);
		
        reportLog("11.5: Site User_1 is displayed in the Existing Users section with highlighted matched users details");
		...
		reportLog("13.0: Select row for Site User_1");
		ratersDetailsPage.selectUserIntoNewUserMerge(Constants.userName);

		reportLog("13.1: Site User_1 is selected");
		...
		reportLog("14.1: Site User_5 is merged with Site User_1 and Site User_5 not displayed in the grid");
		ratersDetailsPage.verifyRaterIsDeletedOrNotDisplayed(Constants.RaterForReview_5);

		reportLog("15.0: Navigate to Site User_1 profile");
		...
		reportLog("15.1: Updated details for Site User_1 are displayed in the profile");
		ratersDetailsPage.verifyUpdatedDetailsShowingInUserProfile(Constants.study,Constants.facility);
		ratersDetailsPage.selectBackButton();
		
		reportLog("16.0: Navigate to Site User_1 view history page");
		ratersDetailsPage.selectRaterWithCompletedStatus(Constants.Rater_1);
		ratersDetailsPage.selectActionControl();
        ratersDetailsPage.clickOnViewHistoryLink();
        
		reportLog("16.1: Appropriate actions are displayed in the View History page for Site User_1"
				+ " (Add to Facility/ Add to Study/Add to Site)");
		ratersDetailsPage.verifyNewEntryDisplayed();
		
		reportLog("Deleting Site User_4 to Restore prequisite" );
		ratersDetailsPage.selectBackButton();
		ratersDetailsPage.applyNameFilterUnderTrackingTab(Constants.RaterForReview_4);
