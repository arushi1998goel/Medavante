...
public abstract class BasePage {
		...
		if (driver.findElements(By.xpath(
			...
			for (WebElement subTab : driver.findElements(By.xpath(
				...
				if (getText(subTab).trim().contains(subTabValue[0])) {
					moveToElement(subTab);
					javascriptButtonClick(subTab);
    ...
    {
   	 waitForAjaxRequestsToComplete();
		waitForElementToBecomeInvisible(By.xpath("//*[contains(@class,'spinner')]"));
    }

    /**
     * MA-Spinner: Disabled
     */
    public void waitUntillFinishSpinnerProcessDisable()
    {
   	 waitForAjaxRequestsToComplete();
		waitForElementToBecomeInvisible(By.xpath("//*[@class='ma-spinner']"));
    }
	...
	@FindBy(how=How.XPATH,using="//ul[@class='dropdown-menu']//a[contains(text(),'Forms Library')]")
	private WebElement formLibrary;
	...
	public FormManagerPage navigateToFormLibrary()
	{
		waitAndClick(configureOption);
		boolean flag = false;
			if (formLibrary.isDisplayed()) {
				javascriptButtonClick(formLibrary);
				flag = true;
			}
		Assert.assertTrue(flag);
		WebDriverWait wait = new WebDriverWait(driver, 30);
		wait.until(ExpectedConditions.invisibilityOfElementLocated(By.xpath("//span[@class='spinner spinning']")));
		return PageFactory.initElements(driver, FormManagerPage.class);
	}
	...
	public void verifyContinueButtonIsDisplayed(){
		moveToElement(continueTrainingBtn);
		Assert.assertTrue(continueTrainingBtn.isDisplayed());
		reportInfo();
	}
