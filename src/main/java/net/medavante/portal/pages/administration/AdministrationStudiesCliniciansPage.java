...
public class AdministrationStudiesCliniciansPage extends BasePage {
	...
	@FindBy(xpath = "//span[text()='Add clinician anyway']")
	private WebElement addClinAnyway;
	
	@FindBy(xpath = "//h4[@class='modal-title bold' and text()='Confirmation']")
	private WebElement cliniConfirmationpopup;
	...
	public void addClinician(String clinicianToBeSelected, String systemRoleToBeSelected) {
		...
		moveToElement(cliniConfirmationpopup);
		if(cliniConfirmationpopup.isDisplayed()) {
			moveToElement(addClinAnyway);
			addClinAnyway.click();
		}
		waitForSpinnerBecomeInvisible(5);
		_normalWait(500);
		WebElement clinName =driver.findElement(By.xpath("//div[contains(@class,'blue-popup')]//div[contains(text(),'"+clinicianToBeSelected+"')]"));
		waitAndClick(clinName);
		_normalWait(100);
