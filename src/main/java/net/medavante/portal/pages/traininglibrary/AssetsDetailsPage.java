...
public class AssetsDetailsPage extends BasePage {
	...
	@FindBy(xpath = "//div[@class='aside-nav-wrap']//span[text()='Training']")
	private WebElement trainingMenuLink;
	
	@FindBy(xpath = "//div[@class='btn-group open']/ul/li/a[contains(text(),'Activate')]")
	private WebElement activateLink;
	...
	public void verifyAssetsListUpdated(String count)
	{
		...
		//System.out.println("Value of i"+ updated);
		_normalWait(2000);
		...
		//System.out.println("Value of j"+ NewList);
	...
	public void activateAddedAsset(String AssetName){
		
		WebElement trainingtobedeleted = driver.findElement(ByLocator("//a[text()='"+AssetName+"']/ancestor::td/preceding-sibling::td[contains(@class,'k-checkbox-cell')]"));
		clickOn(trainingtobedeleted);
		waitAndClick(actionbtn);
		clickOn(activateLink);
		waitUntillFinishSpinnerProcessDisable();
	}
	
	public TrainingDetailsPage navigateToTrainingMenuLink()
	{
		waitUntillFinishSpinnerProcessDisable();
		clickOn(trainingMenuLink);
		waitUntillFinishSpinnerProcessDisable();
		reportInfo();
		return PageFactory.initElements(driver, TrainingDetailsPage.class);
	}
