...
public class RatersDetailsPage extends BasePage {
	...
	@FindBy(xpath = "//p[contains(text(),'Study')]//parent::div//div[contains(@class,'item ')]")
	...
	@FindBy(xpath = "//*[text()='Site:']//parent::div//following-sibling::div//div[contains(@class,'item ')]")
	...
	@FindBy(xpath = "//span[text()='Add Multiple Users']")
	...
	@FindBy(xpath = "//div[contains(text(),'These users will not be added! Are you sure?')]")
	...
	@FindBy(xpath = "//div[contains(@class,'form-fields-box')]//div[@class='input-container']")
	...
	private WebElement onBoardingPage;
	...
	@FindBy(xpath = "//strong[contains(text(),'Raters: ')]")
	...
	@FindBy(xpath = "(//div[@class='btn-group'])[2]/div")
	private WebElement actionControl;

	@FindBy(xpath = "(//ul[@class='dropdown-menu pull-right'])[2]/li/a[text()='View History']")
	...
	@FindBy(xpath = "//div[@class='form-fields-box ng-star-inserted']//span[@class='k-input']")
	private WebElement siteRoleDropDown;

	@FindBy(xpath = "//div[contains(@class,'k-popup k-list-container')]//ul/li[contains(text(),'Site User')]")
	...
	@FindBy(xpath = "//div[@class='sidenav-area']//ul/li/span[text()='General Training']")
	private WebElement generalTraining;
	
	@FindBy(xpath = "//div[@class='form-fields-box ng-star-inserted']//span[@class='k-input']")
	private List<WebElement> siteUserRole;
	
	@FindBy(xpath="//ul[@class='aside-nav-list']/li[@routerlink='./on-boarding']")
	private WebElement onBoardingMenu;
	
	@FindBy(xpath = "//div[@class='btn-group open']/ul/li/a")
	private List<WebElement> allLinkUnderAction;
		...
		if (driver.findElements(By.xpath("//*[contains(@id,'cdk-overlay')]")).size() > 0) {
			...
			for (WebElement site : siteLists) {
				if (site.getText().trim().contains("- "+siteName)) {
					...
					_normalWait(2000);
					break;
				}else if (site.getText().trim().contains(siteName)) {
					javascriptButtonClick(site);
					_normalWait(2000);
					break;
				} {
	...
	public void selectRater(String ratername) {
		...
		WebElement ratertobeselected = driver
				.findElement(ByLocator("//*[contains(text(),'" + ratername + "')]/ancestor::td/preceding::td"));
...
//				.findElement(ByLocator("//h3[text()='Studies']/..//strong[contains(text(),'" + Study + "')]"));
	...
	public void verifyUserUnderReviewWithHighlightedAndDisplayed(String value, String HighlightedColor) {
		WebElement detailtoverifed = driver.findElement(
				ByLocator("//table[@class='raters-list indented-table']//*[text()='" + value + "']"));
	...
	public void verifyFieldHighlightedForUnderReviewUser(String value, String HighlightedColor) {
		WebElement detailtoverify = driver
				.findElement(ByLocator("//table[@class='raters-list indented-table']//*[text()='" + value + "']"));
	...
	public void clickOnBackButton() {
		moveToElement(BackButton);
		javascriptButtonClick(BackButton);
	}
	...
	public void selectStudyFromStudyDropDownOptions(String study) {
		...
		_normalWait(1000);
		for (WebElement studyNametoselect : studiesList) {
			//System.out.println(studyNametoselect.getText());
			if (studyNametoselect.getText().trim().contains(study)) {
				moveToElement(studyNametoselect);
	...
	public void verifyRaterIsSelected(String Rater) {
		WebElement chkboxtobeverified = driver
				.findElement(ByLocator("//*[text()='" + Rater + "']/ancestor::td/preceding::td/input"));
		Assert.assertTrue(chkboxtobeverified.isSelected());
		reportInfo();
	}
	...
	public void verifyClinicians_RatersPage() {
		//clickOn(generalTraining);
		waitUntillFinishSpinnerProcessDisable();
		moveToElement(onBoardingPage);
		//String raters_clinicians_text = Clinicians_Raters.getText();
		//Assert.assertTrue(raters_clinicians_text.contains(Constants.Raters_Clinicians));
		Assert.assertTrue(isElementDisplayed(onBoardingPage));
	}
	...
	public String navigateToViewHistoryOfCreatedSiteRater() {
		...
		javascriptButtonClick(viewHistoryButton);
		return name;
	...
	public void selectSystemRoleOnAddMultipleUserPopUp() {
		for(WebElement role : siteUserRole) {
			waitAndClick(role);
			waitAndClick(siteRoleDropDownOption);
		}
	}
	
	public void verifyONBoardingMenuSelected() {
		boolean flag = false;
		if(onBoardingMenu.getAttribute("class").equalsIgnoreCase("selected")) {
			moveToElement(onBoardingMenu);
			flag =true;
		}
		Assert.assertTrue(flag);
	}
	...
	/**
	 * verify site drop down is empty
	 */
	public void verifySiteDropDownIsEmpty() {
		waitAndClick(inputSiteContainerField);		
		boolean flag = true;
			if (sitesList.size() > 0) {
				flag = false;
	...
	/**
	 * verify system role drop down is displayed with selected data
	 * @param Siterole
	 */
	public void verifySystemRoleDropDownIsDisplayedWithSelectedData(String Siterole) {
		for(WebElement role : siteUserRole) {
			String txt=role.getText();
			boolean flag =false;
			if(txt.contains(Siterole)) {
				flag=true;
			}
			Assert.assertTrue(flag);		
		}
	...
	public void clickOnLinkUnderActionControl(String linkName) {
		boolean flag = false;
		for(WebElement link : allLinkUnderAction) {
	...
	@FindBy(xpath = "//*[contains(@id,'mat-dialog')]//span[text()='Assign General Training']")
	private WebElement assignGeneralTrainingPopUp;
	
	@FindBy(xpath = "//*[contains(@id,'mat-dialog')]//input[contains(@class,'form-control')]")
	private WebElement searchTrainingField;
	
	public void verifyAssignGeneralTrainingPopUp() {
		boolean flag = false;
		waitUntillFinishSpinnerProcessDisable();
		if(assignGeneralTrainingPopUp.isDisplayed()) {
			moveToElement(assignGeneralTrainingPopUp);
			flag = true;
		}
		Assert.assertTrue(flag);
	}
	
	public void assignTrainingToUser(String training) {
		inputText(searchTrainingField, training);
		WebElement trainLink = driver.findElement(By.xpath("//*[contains(@id,'mat-dialog')]//span[text()='"+training+"']"));
		waitAndClick(trainLink);
		_normalWait(500);
	}
	
	public void clickOnSaveButtonOnAssignGeneraltrainingPopUp(String UN, String PWD) {	
			moveToElement(saveButtonOnAssignToFacilityOrStudy);
			waitAndClick(saveButtonOnAssignToFacilityOrStudy);
			waitUntillFinishSpinnerProcessDisable();
			inputText(userNameInputField, UN);
